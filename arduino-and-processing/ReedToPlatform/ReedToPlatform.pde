...
boolean reedValue=false,cReedValue=false;
float angle;
float easing=0.05;
...
void draw() {
  ...
  //  fill(#030CFF);  //顯示藍色
  //}
  text(reedValue?"CLOSE":"OPEN",width/20,height*9/10);
  ...
  //text(int(val),width/2,height/2);
   if (reedValue!=cReedValue)  //若資料的值有改變
  {
    Thread t=new Thread(new Runnable() {
      public void run() {
        setDataPointCht(reedValue?1:0,deviceId,sensorIdR); //傳資料到CHT Platform
      }
    }
    );
    t.start();
  ...
  cReedValue=reedValue;  //將最新的溫度數值記錄在變數
}
